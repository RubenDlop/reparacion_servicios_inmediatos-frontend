...
<div class="card">
  <div class="head">
    ...
    <div>
      ...
      <div class="meta">Asignación #{a?.id ?? a?.assignment_id ?? '—'} • {timeAgo(a?.updated_at ?? a?.updatedAt)}</div>
    </div>
  ...
  <div class="pills">
    <div class="pill"><span aria-hidden="true">{@html I.pin}</span>{a?.city ?? a?.location ?? a?.address ?? 'Ubicación no especificada'}</div>
  ...
  <div class="ctaRow">
    ...
    {#if (a?.status ?? '').toLowerCase() === 'pending'}
      <button class="btn ghost" disabled={switchingId===(a?.id ?? a?.assignment_id)} on:click={onAccept}>
        {switchingId===(a?.id ?? a?.assignment_id) ? 'Aceptando…' : 'Aceptar trabajo'}
      </button>
    {/if}
    {#if ['in_process','en_proceso','assigned'].includes((a?.status||'').toLowerCase())}
      <button class="btn ghost" disabled={switchingId===(a?.id ?? a?.assignment_id)} on:click={onFinish}>
        {switchingId===(a?.id ?? a?.assignment_id) ? 'Finalizando…' : 'Finalizar'}
      </button>
    {/if}
  </div>
  ...
  <details>
    ...
    <div class="block">
      ...
      <div class="muted">{a?.description || 'Sin descripción proporcionada.'}</div>
    </div>

    <div class="block">
      <h5>Cliente</h5>
      <div class="muted">{a?.customer?.full_name ?? 'Cliente'} · {a?.customer?.phone ?? '—'}</div>
    </div>
    ...
    <div class="block">
      ...
      {#if getLat(a) != null && getLng(a) != null}
        ...
        {/key}
      {:else}
        <div class="miniMap">Sin coordenadas — {a?.address ?? a?.city ?? '—'}</div>
      {/if}
      <div class="muted">{a?.address ?? a?.city ?? '—'}</div>
    </div>
